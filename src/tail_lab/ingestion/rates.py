...
``https://api.stlouisfed.org/fred/series/observations`` -- free but keyed,
unlike every other adapter in this package. The key is now provisioned
(`HUMAN_TODO.md`, done 2026-08-17) as the ``FRED_API_KEY`` repo secret /
local ``.env`` entry, read through :class:`tail_lab.config.Settings`
(never ``os.environ`` directly -- `ARCHITECTURE.md` "Where new code goes").
Ingests the Treasury curve (``DGS1MO``...``DGS30``), SOFR and the fed
funds effective rate (``DFF``).

The point-in-time hazard here differs from every keyless adapter so far:
FRED series *can* be revised after first publication, so pulling only the
latest value is a look-ahead bug, not a simplification
(`docs/DATA_CONTRACTS.md` #3). The adapter therefore always requests
FRED's ALFRED-style full vintage history -- ``realtime_start``/
``realtime_end`` spanning FRED's own "since the beginning of time"
sentinel range, confirmed to work on the free key (`docs/DATA_SOURCING.md`
§2) -- rather than the single-vintage default. This yields one row per
``(series, obs_date, vintage)`` FRED has ever reported, each carrying its
own ``vintage_date`` (FRED's ``realtime_start``), rather than relying on
the ``ingest_date`` Delta partition to capture revisions. Treasury/SOFR/
fed-funds levels are practically never revised, so in practice this ships
one row per ``obs_date`` -- but the adapter validates whatever FRED
actually returns rather than assuming that.
...
- :func:`fetch_series_observations_raw` -- the HTTP call, exercised only by
  ``make ingest-rates`` (human/manually run), never by CI/pytest.
- :func:`ingest_rates` -- orchestrates fetch -> parse -> validate/
...
family lands in a single long-format ``rates`` dataset keyed by
``(series_id, obs_date, vintage_date)`` -- they are fetched together, read
together as a panel, and share an identical source format, following
``ingestion/cboe_strategy.py``'s precedent for the same reasoning.
"""
...
def ingest_rates(
    ...
    *,
    ...
    (matches ``ingest_cboe_strategy``: a partial run must not half-overwrite
    the previous complete panel).
    ...
    ``api_key`` raise -- FRED's endpoint is keyed, unlike every other
    ...
    for series_id in resolved:
        if raw is not None and series_id in raw:
            ...
            if not api_key:
                raise ValueError(
                    "ingest_rates needs a FRED api_key for a live fetch of "
