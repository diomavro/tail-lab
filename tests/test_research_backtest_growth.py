...
def test_time_average_growth_pinned_case() -> None:
    ...
    ~1.002-year window, with a hedge that realized -5,000 net over the same span:
    ...
    g = time_average_growth(price_path, -5000.0, wealth=100_000.0)
    assert g is not None
    assert g == pytest.approx(0.14811586576352267)


def test_time_average_growth_zero_hedge_matches_benchmark_only_growth() -> None:
    """A zero hedge (the default): the combined growth collapses to the
    benchmark's own geometric growth, ln(121/100) / years = 0.19022974411764837."""
    price_path = [(dt.date(2024, 1, 1), 100.0), (dt.date(2025, 1, 1), 121.0)]
...
def test_time_average_growth_only_endpoints_of_price_path_matter() -> None:
    ...
    assert time_average_growth(endpoints_only, -5000.0, wealth=100_000.0) == time_average_growth(
        with_interior, -5000.0, wealth=100_000.0
    )
...
def test_time_average_growth_undefined_cases_return_none() -> None:
    ...
    assert time_average_growth(flat_path, -200_000.0, wealth=100_000.0) is None
