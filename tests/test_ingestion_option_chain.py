...
def test_a_wholly_frozen_feed_is_refused_not_no_opped(tmp_path: Path) -> None:
    """The silent loss with no fresh minority to give it away.

    Measured 2026-09-23..24: Cboe's old CDN host froze at 03:55 UTC on the
    23rd (it had moved to a new one), so EVERY chain kept reporting session 2026-09-22. That session
    was already captured, so each sweep no-opped and exited 0 while the 23rd
    was lost for good -- the stale-majority guard above cannot fire, because
    there is no off-session row at all. Only a witness outside Cboe can see
    it: the market completed a newer session than anything Cboe is serving.
    """
    live = [f"SYM{i:02d}" for i in range(24)]
    store = DeltaLakeStore(tmp_path)
    frozen = _fetcher(**{n: _payload(_put(95.0), symbol=n, day="2026-08-25") for n in live})
    ingest_option_chain(store, live, fetch=frozen)

    with pytest.raises(IncompleteSweepError, match="Cboe's feed is behind"):
        ingest_option_chain(store, live, fetch=frozen, market_session=dt.date(2026, 8, 26))


def test_a_market_holiday_is_still_a_quiet_no_op(tmp_path: Path) -> None:
    """The reason the witness is a market data source rather than a calendar
    or the wall clock: on a holiday (Labor Day 2026-09-07 served 2026-09-04
    all day) the reference did not trade either, so it agrees with Cboe and
    the re-run stays green. An alarm that fires every holiday is one people
    learn to ignore, on the one job whose alarms must be believed."""
    live = [f"SYM{i:02d}" for i in range(24)]
    store = DeltaLakeStore(tmp_path)
    friday = _fetcher(**{n: _payload(_put(95.0), symbol=n, day="2026-09-04") for n in live})
    ingest_option_chain(store, live, fetch=friday)

    rerun = ingest_option_chain(store, live, fetch=friday, market_session=dt.date(2026, 9, 4))

    assert rerun.committed is False


def test_a_behind_feed_still_captures_a_session_the_lake_lacks(tmp_path: Path) -> None:
    """Cboe lagging the market is only fatal when it makes the write a no-op.
    If the session it serves is NOT yet in the lake, those are real quotes for
    a real session and refusing them would lose it twice over."""
    live = [f"SYM{i:02d}" for i in range(24)]
    store = DeltaLakeStore(tmp_path)
    behind = _fetcher(**{n: _payload(_put(95.0), symbol=n, day="2026-08-25") for n in live})

    result = ingest_option_chain(store, live, fetch=behind, market_session=dt.date(2026, 8, 26))
...
def test_a_lagging_witness_cannot_raise_an_alarm(tmp_path: Path) -> None:
    ...
    behind -- even when that older session's partition is genuinely missing
    (as 2026-09-23's is in production). Only a witness strictly AHEAD of Cboe
    may refuse; anything else is a false alarm with a backwards message."""
    live = [f"SYM{i:02d}" for i in range(24)]
    store = DeltaLakeStore(tmp_path)
    today = _fetcher(**{n: _payload(_put(95.0), symbol=n, day="2026-08-26") for n in live})
    ingest_option_chain(store, live, fetch=today)

    rerun = ingest_option_chain(store, live, fetch=today, market_session=dt.date(2026, 8, 25))

    assert rerun.committed is False


def test_a_partial_cdn_rollback_after_capture_is_not_an_alarm(tmp_path: Path) -> None:
    """The stale-majority guard's rollback twin: most chains step back to an
    already-captured session while a few still carry the newer one -- which
    is ALSO already captured. Nothing is discarded, so refusing would claim a
    ...
    ingest_option_chain(
        store,
        live,
        fetch=_fetcher(**{n: _payload(_put(95.0), symbol=n, day="2026-08-26") for n in live}),
    )
    rolled_back = set(live[:21])
    mixed = _fetcher(
        **{
            n: _payload(
                _put(95.0), symbol=n, day="2026-08-25" if n in rolled_back else "2026-08-26"
            )
            for n in live
        }
    )
