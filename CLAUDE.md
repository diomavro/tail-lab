...
- **Agent branches auto-merge.** `.github/workflows/automerge.yml` squash-merges any `agent/*` branch PR automatically once CI is fully green. Pushing to an `agent/*` branch is effectively pushing to main-after-CI; use a differently-named branch when a human should review first. CI's `constitution-guard` job fails any `agent/*` PR that touches `README.md`, `ARCHITECTURE.md`, `docs/STANDARDS.md`, `docs/AGENT_MISSION.md`, `docs/END_STATE.md`, or `docs/adr/` — those changes need a human-merged PR.
...
- `research/` currently has `metrics/downside_beta.py`, `option_pricer.py`, `vix_stretch.py`, `backtest/put_roll.py`, `regimes/timeline.py`.
