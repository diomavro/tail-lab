...
.PHONY: setup lint format typecheck import-lint test check cov-floors ingest-vix ingest-ohlcv ingest-options-expiry ingest-cboe-strategy ingest-rates ingest-credit ingest-option-quotes residual skew api frontend clean
...
help:
	...
	@echo "  ingest-options-expiry  Live options-expiry chain fetch (Cboe, Yahoo fallback) -> bronze for SYMBOL (default AAPL; network; not run in CI)"
